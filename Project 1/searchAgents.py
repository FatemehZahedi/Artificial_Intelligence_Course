...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        Goal = (len(state[1]) == len(self.corners))
        return Goal
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    pos, visitedcorner = state
    
    heuristic = 0
    x, y = pos

    unvisitedcorner = [i for i in corners if i not in list(visitedcorner)]

    while len(unvisitedcorner) > 0:
        distances = []
        for corner in list(unvisitedcorner):
            xnew, ynew = corner
            distance = abs(xnew - x) + abs(ynew -y)
            distances.append((distance,corner))
        
        distances.sort(key = lambda x : x[0])
        disindex = distances[0][1]
        dis = distances[0][0]
        heuristic += dis
        x, y = disindex
        unvisitedcorner.remove(disindex)
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    foodlists = foodGrid.asList()
    initialState = problem.startingGameState

    if len(foodlists) is 0:
        return 0
    else:
        distances = []
        for food in foodlists:
            distances.append(mazeDistance(position, food, initialState))
