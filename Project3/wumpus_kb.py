...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for i in range(len(tvec)):
        if i is 0:
            if tvec[i]:
                axiom_list.append(percepts[i] + str(t))
            else:
                axiom_list.append('~' + percepts[i] + str(t))
        else:
            if tvec[i]:
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} & (~{1} & ~{2})'.format(state_loc_str(x, y, 0), pit_str(x, y), wumpus_str(x, y))
    return axiom_str
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    pits = []
    pits_string = ''
    neighboring = [(x,y),(x-1,y),(x,y-1),(x+1,y),(x,y+1)]
    for (xcheck, ycheck) in neighboring:
        if xcheck >= xmin and xcheck <= xmax and ycheck >= ymin and ycheck <= ymax:
            pits.append(pit_str(xcheck, ycheck))
    pits_string = ' | '.join(pits)
    axiom_str = '{0} <=> ({1})'.format(breeze_str(x, y),pits_string)
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for (xcheck, ycheck) in neighboring:
        if xcheck >= xmin and xcheck <= xmax and ycheck >= ymin and ycheck <= ymax:
            wumpuses.append(wumpus_str(xcheck, ycheck))
    wumpuses_string = ' | '.join(wumpuses)
    axiom_str = '{0} <=> ({1})'.format(stench_str(x, y),wumpuses_string)
    return axiom_str
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for x in range(xmin, xmax+1):
        for y in range(ymin, ymax+1):
            if (x is not xmax) or (y is not ymax):
                wumpus_list.append(wumpus_str(x,y) + ' | ')
            else:
                wumpus_list.append(wumpus_str(x,y))
    axiom_str = ''.join(wumpus_list)
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for (x, y) in xy:
        ...
        for(xn, yn) in xy:
            if (xn, yn) != (x, y):
                notwumpus.append('~' + wumpus_str(xn, yn))
        wumpus.append('({0} >> ({1}))'.format(wumpus_str(x, y),' & '.join(notwumpus)))
    axiom_str = ' & '.join(wumpus)
    return axiom_str
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    "*** YOUR CODE HERE ***"
    axiom_list = []
    for xcheck in range(xmin, xmax + 1):
        for ycheck in range(ymin, ymax + 1):
            if (xcheck, ycheck) != (xi, yi):
                axiom_list.append('~' + state_loc_str(xcheck, ycheck, t))
    axiom_str = '{0} >> ({1})'.format(state_loc_str(xi, yi, t),' & '.join(axiom_list))
    return axiom_str
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    for head in headings:
        if head.upper() != heading.upper():
            notheading.append('~Heading' + head + str(t))
        else:
            realhead = head
    axiom_str = 'Heading{0}{1} & ({2})'.format(realhead, str(t), ' & '.join(notheading))
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    for xcheck in range(x-1, x+2):
        for ycheck in range(y-1, y+2):
            if xcheck >=xmin and xcheck <= xmax and ycheck >= ymin and ycheck <= ymax:
                if xcheck is x - 1 and ycheck is y:
                    axiom_list.append('({0} & ({1} & {2}))'.format(state_loc_str(xcheck,ycheck,t),\
                     state_heading_east_str(t), action_forward_str(t)))
                elif xcheck is x + 1 and ycheck is y:
                    axiom_list.append('({0} & ({1} & {2}))'.format(state_loc_str(xcheck,ycheck,t),\
                     state_heading_west_str(t), action_forward_str(t)))
                elif ycheck is y - 1 and xcheck is x:
                    axiom_list.append('({0} & ({1} & {2}))'.format(state_loc_str(xcheck,ycheck,t),\
                     state_heading_north_str(t), action_forward_str(t)))
                elif ycheck is y + 1 and xcheck is x:
                    axiom_list.append('({0} & ({1} & {2}))'.format(state_loc_str(xcheck,ycheck,t),\
                     state_heading_south_str(t), action_forward_str(t)))
                elif xcheck is x and ycheck is y:
                    axiom_list.append('({0} & ({1} | {2} | {3} | {4} | ~{5} | {6}))'.format(state_loc_str(xcheck,ycheck,t),\
                     action_turn_left_str(t), action_turn_right_str(t), action_grab_str(t), action_shoot_str(t),\
                          action_forward_str(t), percept_bump_str(t+1)))
    
    axiom_str = '{0} <=> ({1})'.format(state_loc_str(x, y, t+1), ' | '.join(axiom_list))                
...
def axiom_generator_heading_north_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (({1} & ({6} | {7} | {8} | {9} | {10})) | ({4} & {3}) | ({5} & {2}))'.format(state_heading_north_str(t+1), state_heading_north_str(t), action_turn_right_str(t),\
        action_turn_left_str(t), state_heading_east_str(t), state_heading_west_str(t), action_grab_str(t), action_shoot_str(t), action_forward_str(t), percept_bump_str(t+1), action_wait_str(t))
    return axiom_str
...
def axiom_generator_heading_east_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (({1} & ({6} | {7} | {8} | {9} | {10})) | ({4} & {2}) | ({5} & {3}))'.format(state_heading_east_str(t+1), state_heading_east_str(t), action_turn_right_str(t),\
        action_turn_left_str(t), state_heading_north_str(t), state_heading_south_str(t), action_grab_str(t), action_shoot_str(t), action_forward_str(t), percept_bump_str(t+1), action_wait_str(t))
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (({1} & ({6} | {7} | {8} | {9} | {10})) | ({4} & {2}) | ({5} & {3}))'.format(state_heading_south_str(t+1), state_heading_south_str(t), action_turn_right_str(t),\
        action_turn_left_str(t), state_heading_east_str(t), state_heading_west_str(t), action_grab_str(t), action_shoot_str(t), action_forward_str(t), percept_bump_str(t+1), action_wait_str(t))
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (({1} & ({6} | {7} | {8} | {9} | {10})) | ({4} & {3}) | ({5} & {2}))'.format(state_heading_west_str(t+1), state_heading_west_str(t), action_turn_right_str(t),\
        action_turn_left_str(t), state_heading_north_str(t), state_heading_south_str(t), action_grab_str(t), action_shoot_str(t), action_forward_str(t), percept_bump_str(t+1), action_wait_str(t))
    return axiom_str
...
def axiom_generator_heading_only_north(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_north_str(t), state_heading_south_str(t), state_heading_east_str(t), state_heading_west_str(t))
    return axiom_str
...
def axiom_generator_heading_only_east(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_east_str(t), state_heading_south_str(t), state_heading_north_str(t), state_heading_west_str(t))
    return axiom_str
...
def axiom_generator_heading_only_south(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_south_str(t), state_heading_north_str(t), state_heading_east_str(t), state_heading_west_str(t))
    return axiom_str
...
def axiom_generator_heading_only_west(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_west_str(t), state_heading_south_str(t), state_heading_north_str(t), state_heading_east_str(t))
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    action_list = [action_forward_str(t), action_wait_str(t), action_shoot_str(t), action_climb_str(t), action_turn_right_str(t), action_turn_left_str(t)]
    action = []
    for i in range(len(action_list)):
        otheractions = []
        for j in range(len(action_list)):
            if i != j:
                otheractions.append('~' + action_list[j])
        action.append('({0} <=> ({1}))'.format(action_list[i],' & '.join(otheractions)))
    axiom_str = ' & '.join(action)
    return axiom_str
