...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        """
        "*** YOUR CODE HERE ***"
        goal_lists = self.goals
        distance = []
        for goal in goal_lists:
            distance.append(manhattan_distance_with_heading(node.state, goal))
    ...
    def actions(self, state):
        ...
        y = state[1]
        heading = state[2]
        possibleloc = [(x, y + 1), (x - 1, y), (x, y - 1), (x + 1, y)]

        for i in range(len(possibleloc)):
            if heading == i and possibleloc[i] in self.allowed:
    ...
    def result(self, state, action):
        ...
        heading = state[2]
        possibleloc = [(x, y + 1), (x - 1, y), (x, y - 1), (x + 1, y)]
        possibleaction = ['Forward', 'TurnRight', 'TurnLeft']
        headinglist = []
        loc = []
        for iact in range(len(possibleaction)):
            if action == possibleaction[iact]:
                if iact == 0:
                    loc = possibleloc
                    headinglist = [0, 1, 2, 3]
                elif iact == 1:
                ...
                for i in range(len(headinglist)):
                    if heading == i:
                        return (loc[i][0], loc[i][1], headinglist[i])
...
class PlanShotProblem(search.Problem):
    ...
    def h(self,node):
        ...
        wumpusloc = self.goals
        wumpusadjacent = self.allowed
        distance = []
        wumpusadj = []

        for wumpus in wumpusloc:
            for loc in wumpusadjacent:
                if (loc[0] is wumpus[0]) or (loc[1] is wumpus[1]):
                    wumpusadj.append(loc)
        
        for goal in wumpusadj:
            distance.append(manhattan_distance_with_heading(node.state, goal))
    ...
    def goal_test(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        wumpuslocation = self.goals
        x = state[0]
        y = state[1]
        wumpusadj = [(x-1, y), (x, y-1), (x+1, y), (x, y+1)]
        trueshothead = [1, 2, 3, 0]

        if (x, y) in wumpuslocation:
            return False

        for (xw, yw) in wumpuslocation:
            for i in range(len(wumpusadj)):
                if (xw is wumpusadj[i][0]) and (yw is wumpusadj[i][1]) and (state[2] is trueshothead[i]):
                    return True
        return False
